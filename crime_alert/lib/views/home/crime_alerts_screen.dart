...
//             print(
//                 "📍 Creating marker at ($latitude, $longitude) for crime: $crimeType");
...
//         } else {
//           print(
//               "⚠ Skipping report ${doc.id} due to missing latitude/longitude");
//         }
//       }
//     }, onError: (error) {
...
//     try {
//       final apiKey = "AIzaSyBZFVz_dHpmtiJbjIyipcJgiCQ173xYylE"; // Replace with your API Key
//       final url = Uri.parse(
  ...
  void _setupReportsStream() {
    ...
    _reportsSubscription = FirebaseFirestore.instance
      ...
      for (var doc in snapshot.docs) {
        ...
        // Check if location field exists and geocode it
        LatLng? markerPosition;
        if (data['location'] != null && data['location'].isNotEmpty) {
          markerPosition = await _getLatLngFromAddress(data['location']);
        }

        // Fallback to latitude/longitude if location geocoding fails or is missing
        if (markerPosition == null &&
          ...
          try {
            ...
            print(
                "⚠ Falling back to Firestore latitude/longitude ($latitude, $longitude) for report ${doc.id}");
          } catch (e) {
            print("❌ Error parsing fallback coordinates (${doc.id}): $e");
            continue; // Skip this report if coordinates are invalid
          }
        }
        ...
        if (markerPosition != null) {
          try {
            ...
            print(
                "📍 Creating marker at (${markerPosition.latitude}, ${markerPosition.longitude}) for crime: $crimeType");
            ...
            print("❌ Error creating marker for report (${doc.id}): $e");
          }
        } else {
          print(
              "⚠ Skipping report ${doc.id} due to missing or invalid location/coordinates");
        }
      }
    }, onError: (error) {
      print("❌ Error loading crime reports from Firestore: $error");
